...
class CloggingApp:
    def __init__(self, root):
        ...
        tk.Label(control_frame, text="Configuration", font=("Helvetica", 16, "bold"), bg="#e1e1e1").pack(pady=20)
        
        # --- CHECKBOXES ---
        tk.Label(control_frame, text="Active Methods:", bg="#e1e1e1", font=("Arial", 11, "bold")).pack(anchor="w", padx=10)
        ...
        tk.Checkbutton(control_frame, text="Hydraulic Anomaly (Static)", variable=self.vars["static"], bg="#e1e1e1", font=("Arial", 10)).pack(anchor="w", padx=20)
        tk.Checkbutton(control_frame, text="Spectral Composite (Hybrid)", variable=self.vars["composite"], bg="#e1e1e1", font=("Arial", 10)).pack(anchor="w", padx=20)
        tk.Checkbutton(control_frame, text="Pure Turbulence (Control)", variable=self.vars["turbulence"], bg="#e1e1e1", font=("Arial", 10)).pack(anchor="w", padx=20)
        
        # AI Button (Blue to make it stand out)
        tk.Checkbutton(control_frame, text="AI / Machine Learning", variable=self.vars["ml"], bg="#e1e1e1", fg="blue", font=("Arial", 10, "bold")).pack(anchor="w", padx=20)

        ttk.Separator(control_frame, orient='horizontal').pack(fill='x', pady=20)
        
        # --- SPEED SLIDER ---
        tk.Label(control_frame, text="Simulation Speed:", bg="#e1e1e1", font=("Arial", 11, "bold")).pack(anchor="w", padx=10)
        self.speed_var = tk.IntVar(value=20) 
        self.speed_slider = tk.Scale(control_frame, from_=1, to=200, orient=tk.HORIZONTAL, variable=self.speed_var, bg="#e1e1e1", highlightthickness=0)
        self.speed_slider.pack(fill=tk.X, padx=10)
        
        ttk.Separator(control_frame, orient='horizontal').pack(fill='x', pady=20)

        btn_spectrum = tk.Button(control_frame, text="🔍 View Live Spectrum (Log-Log)", 
        ...
        tk.Label(control_frame, text="Linear Prediction (Static):", bg="#e1e1e1", font=("Arial", 10, "bold")).pack(anchor="w", padx=10)
        self.pred_lbl = tk.Label(control_frame, text="Waiting...", bg="black", fg="#00ff00", font=("Courier", 14), height=2, relief="sunken")
        self.pred_lbl.pack(fill=tk.X, padx=10, pady=5)

        # ==========================================
        # NEW: COMPOSITE PREDICTION (TRAFFIC LIGHT)
        # ==========================================
        tk.Label(control_frame, text="Composite Prediction (ETA):", bg="#e1e1e1", font=("Arial", 10, "bold")).pack(anchor="w", padx=10, pady=(15, 0))
        self.lbl_traffic_light = tk.Label(control_frame, text="System Stable", bg="#4CAF50", fg="white", font=("Arial", 12, "bold"), height=2)
        self.lbl_traffic_light.pack(fill=tk.X, padx=10, pady=5)

        # ==========================================
        # NEW: PHYSICS DASHBOARD (SPECTRAL SLOPE)
        # ==========================================
        tk.Label(control_frame, text="Spectral Slope (β):", bg="#e1e1e1", font=("Arial", 10, "bold")).pack(anchor="w", padx=10, pady=(15, 0))
        self.lbl_slope = tk.Label(control_frame, text="β = 0.00", bg="white", fg="black", font=("Courier", 14, "bold"), relief="sunken")
        ...
        tk.Label(legend_frame, text="Healthy: < -2.5", fg="green", bg="#e1e1e1", font=("Arial", 8)).pack(side=tk.LEFT)
        tk.Label(legend_frame, text="Clogged: > -1.5", fg="red", bg="#e1e1e1", font=("Arial", 8)).pack(side=tk.RIGHT)
        
        ttk.Separator(control_frame, orient='horizontal').pack(fill='x', pady=20)

        # --- LOAD BUTTONS ---
        tk.Label(control_frame, text="Data Source:", bg="#e1e1e1", font=("Arial", 11, "bold")).pack(anchor="w", padx=10)
        self.load_file_btn = tk.Button(control_frame, text="📄 Load Single File (Excel/CSV)", command=self.load_file, bg="#5c5c5c", fg="white")
        self.load_file_btn.pack(fill=tk.X, padx=10, pady=5)
        self.load_dir_btn = tk.Button(control_frame, text="📂 Load Folder (Batch)", command=self.load_folder, bg="#5c5c5c", fg="white")
        self.load_dir_btn.pack(fill=tk.X, padx=10, pady=5)
        
        self.status_lbl = tk.Label(control_frame, text="No source selected", fg="#666", bg="#e1e1e1")
        self.status_lbl.pack(pady=5)
        
        self.start_btn = tk.Button(control_frame, text="▶ START SIMULATION", command=self.start_simulation, bg="#007acc", fg="white", font=("Arial", 12, "bold"), state="disabled")
    ...
    def load_file(self):
        ...
        filename = filedialog.askopenfilename(filetypes=[("Data Files", "*.csv *.xlsx")])
        if filename:
    ...
    def start_simulation(self):
        ...
        if total_plots == 1: self.axes = [self.axes]
        ...
        for i, method in enumerate(self.active_methods):
            ...
            if method == "composite": ax.set_yscale('log')
            self.method_axes[method] = ax

        self.fig.tight_layout()
        self.canvas.draw()

        self.data = {'time': [], 'flow': [], 'static': [], 'composite': [], 'turbulence': []}
    ...
    def stop_simulation(self):
        ...
        self.start_btn.config(text="▶ RESTART", command=self.start_simulation, bg="#007acc")
        ...
        try:
            ...
            for _ in range(loops): 
                ...
                if self.is_calibrating:
                    ...
                    if len(self.baseline_buffer) >= 400:
                        ...
                        print("Calibrated.")
                    results = {'static':0, 'composite':1e-6, 'turbulence':0, 'ml':0, 'spectral_slope':0}
                else:
            ...
            if latest_results:
                ...
                if 'light_color' in latest_results:
                    ...
                    hex_col = {"green": "#4CAF50", "yellow": "#FFC107", "red": "#F44336"}.get(c_col, "gray")
                ...
                if 'spectral_slope' in latest_results:
                    ...
                    if beta < -2.0: self.lbl_slope.config(fg="#4CAF50")
                    elif beta < -1.5: self.lbl_slope.config(fg="#FFC107")
                    else: self.lbl_slope.config(fg="red")

                # C. Update Spectrum Window (Throttled: Every 10th frame)
                self.frame_skipper += 1
                if self.frame_skipper % 10 == 0:
                    if self.spectrum_window and tk.Toplevel.winfo_exists(self.spectrum_window):
                        ...
                        if freqs is not None and len(freqs) > 1:
                            try:
                                self.ax_spec.cla()
                                self.ax_spec.loglog(freqs, power, 'b-', color='#2196F3', alpha=0.8, lw=1.5)
                                ...
                                if slope != 0:
                                    ...
                                    self.ax_spec.loglog(freqs, y_line, 'r--', lw=1.5, alpha=0.8, label=f"β={slope:.2f}")
                                    self.ax_spec.legend(loc="upper right")

                                self.ax_spec.set_title("Live PSD (Kolmogorov)", fontweight='bold')
                                self.ax_spec.set_xlabel("Frequency (Hz)")
                                self.ax_spec.set_ylabel("Energy Density")
                                self.ax_spec.grid(True, which="both", ls="--", alpha=0.5)
            ...
            for method in self.active_methods:
                ...
                ax.cla()
                col = {'static':'green', 'composite':'purple', 'turbulence':'gray', 'ml':'blue'}.get(method, 'black')
                ...
                ax.set_xlim(0, 20000)
                if method == "composite": ax.set_yscale('log')
                if method == "ml": ax.set_ylim(0, 1.1)
                
                if method == 'static' and not self.is_calibrating:
                    ax.axhline(self.detector.critical_threshold, color='red', ls='--', alpha=0.5)
                    # Static Prediction logic could go here if you want it back on graph

                if method == 'composite' and not self.is_calibrating:
                     ax.axhline(self.detector.fft_threshold, color='red', ls='--', alpha=0.5)
    ...
    def open_spectrum_window(self):
        ...
        if self.spectrum_window is not None and tk.Toplevel.winfo_exists(self.spectrum_window):
        ...
        self.ax_spec.grid(True, which="both", ls="--", alpha=0.5) # Grid for log decades
