...
CALIBRATION_MIN_WINDOWS = 60  # ≥60 spectra → robust percentile threshold at high σ
TARGET_ANALYSIS_HZ = 20.0  # Cap expensive FFT/ML work at this rate regardless of fs
...
class CloggingDetector:
    ...
    def __init__(
        ...
        # Throttle expensive per-sample analysis (FFT, ML) to TARGET_ANALYSIS_HZ.
        # At fs=100 this skips 4 of every 5 samples; at fs=20 it runs every sample.
        self._analysis_skip = max(1, int(round(fs / TARGET_ANALYSIS_HZ)))
        ...
        self._sorted_composites: np.ndarray = np.array(
            []
        )  # pre-sorted for O(1) percentile
    ...
    @staticmethod
    def required_calibration_samples(fs: float, window_sec: float = 30.0) -> int:
        """Minimum baseline samples needed to fit ≥CALIBRATION_MIN_WINDOWS spectra.

        With 5% step (window_size // CALIBRATION_STEP_RATIO), N+1 windows fit in
        window_size + N*step samples. We size for N = CALIBRATION_MIN_WINDOWS - 1.
        """
        window_size = max(1, int(window_sec * fs))
        step = max(1, window_size // CALIBRATION_STEP_RATIO)
        return window_size + step * (CALIBRATION_MIN_WINDOWS - 1)
    ...
    def _recalculate_thresholds(self):
        ...
        print(
            f"Thresholds recalculated (σ={self.sigma:.1f}): "
    ...
    def calibrate(self, baseline_data: List[float]) -> None:
        ...
        print(
            f"Calibrated! Baseline composite: "
    ...
    def _calibrate_spectrum(self, spectra_list: list, freqs: np.ndarray) -> None:
        ...
        self._sorted_composites = np.sort(
            baseline_composites
        )  # cached for O(1) percentile
    ...
    def process_sample(
        self, pressure_val: float, time_sec: float
        ...
        # Throttle: at fs > TARGET_ANALYSIS_HZ, return the most recent full result
        # for skipped samples so the UI keeps rendering without re-running FFT/ML.
        if (
            self._last_results is not None
            and (self._sample_count % self._analysis_skip) != 0
        ):
            return self._last_results

        raw_signal = np.array(self.buffer)
        curr_mean = float(np.mean(raw_signal))
        fft_power, freqs, current_norm, split_idx = self._compute_fft_spectrum(
            raw_signal
        )
        ...
        results: Dict[str, Any] = {
            ...
            "composite": self._compute_composite_score(
                fft_power, current_norm, split_idx
            ),
            "turbulence": self._compute_turbulence_score(
                raw_signal, curr_mean, fft_power, split_idx
            ),
    ...
    def _compute_fft_spectrum(self, signal: np.ndarray) -> tuple:
    ...
    def _compute_composite_score(
        ...
        if not self.is_calibrated or self.baseline_spectrum_norm is None:
            ...
            return (
                float(np.sum(fft_power[split_idx:])) / total_power
                if total_power > 1e-10
                else 0.0
            )

        composite_val = float(
            np.sum(np.abs(current_norm - self.baseline_spectrum_norm))
        )
        if composite_val < self.fft_threshold * ADAPTIVE_BASELINE_GUARD:
            self.baseline_spectrum_norm = (
                1.0 - self._baseline_alpha
            ) * self.baseline_spectrum_norm + self._baseline_alpha * current_norm
    ...
    def _compute_turbulence_score(
        ...
        return (
            float(np.sum(fft_pure[split_idx:])) / total_pure
            if total_pure > 1e-10
            else 0.0
        )
    ...
    def _fill_calibrated_results(
        ...
        light_color, status_msg = self.predict_composite_eta(
            time_sec, results["composite"]
        )
        ...
        if not self._enable_models:
            results.update(
                {"models": {}, "ensemble_probability": 0.0, "ml_probability": 0.0}
            )
            return
    ...
    def predict_robust(self) -> Optional[tuple]:
        ...
        if len(scores) > window_len:
            scores_smooth = np.convolve(
                scores, np.ones(window_len) / window_len, mode="valid"
            )
