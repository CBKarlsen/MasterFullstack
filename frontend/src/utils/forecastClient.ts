...
function fitExponential(
	...
	if (validIdx.length < MIN_FORECAST_POINTS)
		return { r2: 0, params: {}, etaRel: null };
	...
	const etaRel =
		k > MIN_GROWTH_RATE && s0 > 0 ? Math.log(critical / s0) / k : null;
...
function fitPowerLaw(
	...
	const etaRel =
		n > MIN_GROWTH_RATE && a > 0 ? (critical / a) ** (1 / n) : null;
	return { r2, params: { a, n }, etaRel };
}

function toAbsoluteEta(
	etaRel: number | null,
	onsetTime: number,
): number | null {
	...
	const fits: Record<string, FitResult> = Object.fromEntries(
		Object.entries(rawFits).map(([name, raw]) => [
			name,
			{
				r2: raw.r2,
				params: raw.params,
				eta_seconds: toAbsoluteEta(raw.etaRel, onsetTime),
			},
		]),
	);
