...
export function updateEntryActualTime(timestamp: string, filename: string, value: string | null): void {
  const log = loadLog();
  const entry = log.find((e) => e.timestamp === timestamp && e.filename === filename);
  if (entry) { entry.actualCloggingTime = value; persistLog(log); }
}
...
export function importFromCsv(csvText: string): number {
  ...
  const existingKeys = new Set(existing.map((e) => `${e.filename}__${e.timestamp}`));
