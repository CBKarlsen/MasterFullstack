...
const CONTAMINATION_OPTIONS = [
	{
		label: "Very clean (1%)",
		value: 0.01,
		hint: "Almost no noise in healthy data",
	},
	{ label: "Typical (5%)", value: 0.05, hint: "Recommended for most datasets" },
	{
		label: "Noisy (10%)",
		value: 0.1,
		hint: "Training data has some anomalies mixed in",
	},
	{
		label: "Very noisy (20%)",
		value: 0.2,
		hint: "Heavily mixed / uncertain healthy data",
	},
];
...
export function IFTrainingPanel({ onTrainComplete }: IFTrainingPanelProps) {
	...
	useEffect(() => {
		axios
			...
			.catch(() => {});
		axios
			.get<IFTrainingInfo>("/api/models/isolation_forest/training-info")
		...
		pollRef.current = setInterval(async () => {
			try {
				const res = await axios.get<TrainingProgress>(
					"/api/models/isolation_forest/training-progress",
				);
				...
				if (state.phase === "complete" && state.result) {
					...
					setTrainingInfo({
						...
						stats: state.result as IFTrainingResult,
					});
					setTraining(false);
					onTrainComplete();
		...
		>
				...
				<div style={{ display: "flex", gap: "8px" }}>
					...
					<button
						...
						style={{
							...
							borderRadius: "6px",
							border: "none",
							background: open ? "#e0f2fe" : "#0284c7",
							color: open ? "#0369a1" : "#fff",
							fontSize: "12px",
							fontWeight: 600,
							cursor: "pointer",
						}}
			...
			{(trainingInfo === null || !trainingInfo.is_user_trained) && !open && (
				...
				>
					...
					<strong>Dormant — not contributing to predictions.</strong> Train it
					on a healthy recording to activate it.
				</div>
			)}
				...
				>
					<div style={{ display: "flex", gap: "24px", flexWrap: "wrap" }}>
						<span>
							✓ Trained on{" "}
							<strong>
								{trainingInfo.stats.n_healthy_samples.toLocaleString()}
							</strong>{" "}
							healthy samples
						</span>
						<span>
							Trees: <strong>{trainingInfo.stats.n_estimators}</strong>
						</span>
						<span>
							Flag rate on training:{" "}
							<strong style={{ color: "#d97706" }}>
								{pct(trainingInfo.stats.flag_rate_on_training)}
							</strong>
						</span>
					</div>
				</div>
			...
			{open && (
				...
				>
					...
					>
						<strong>Unsupervised learning:</strong> Point this at recordings
						where the pipe is <strong>healthy</strong>. It learns the normal
						operating pattern and flags anything that deviates — no clogged
						labels needed.
					</div>
					...
					<div>
						...
						>
							...
							>
								file or folder — healthy recordings work best
							</span>
						</label>
						<select
							value={selectedFile}
					...
					>
						<div>
							...
							>
								...
								>
									lower = more healthy samples used
								</span>
							</label>
					...
					>
						...
						<div>
							...
							>
								Expected noise in healthy data
							</label>
							<div
								style={{ display: "flex", flexDirection: "column", gap: "4px" }}
							>
								{CONTAMINATION_OPTIONS.map((o) => (
									...
									>
										<input
											type="radio"
											name="contamination"
											checked={contamination === o.value}
											onChange={() => setContamination(o.value)}
										/>
										{o.label}
									</label>
								))}
					...
					{training && progress && (
						...
						>
							...
							<div
								style={{ fontSize: "11px", color: "#0284c7", marginTop: "6px" }}
							>
								{progress.message}
							</div>
						</div>
					...
					{result && (
						...
						>
								...
								{[
									{
										...
										value: result.n_healthy_samples.toLocaleString(),
									},
									{ label: "Trees", value: String(result.n_estimators) },
									{
										label: "Flag rate on training",
										value: pct(result.flag_rate_on_training),
									},
								].map(({ label, value }) => (
									<div
										key={label}
							...
							>
								The model has learned what "normal" looks like and will now flag
