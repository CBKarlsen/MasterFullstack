...
export function SummaryCard({ label, value, highlight = false }: SummaryCardProps) {
  return (
    <div style={{
      padding: "14px", backgroundColor: highlight ? "#fef2f2" : "#fff",
      borderRadius: "8px", border: `1px solid ${highlight ? "#fca5a5" : "#e5e7eb"}`, textAlign: "center",
    }}>
      <div style={{ fontSize: "12px", color: "#6b7280", marginBottom: "4px" }}>{label}</div>
      <div style={{ fontSize: "16px", fontWeight: 700, color: highlight ? "#090303" : "#111827" }}>{value}</div>
    </div>
  );
}
...
export function CrossingCard({ label, time, threshold, color }: CrossingCardProps) {
...
export function MLStatCard({ label, value, highlight = false }: MLStatCardProps) {
  return (
    <div style={{
      padding: "12px 14px",
  ...
  const perModelStats = useMemo(() => {
    ...
    return modelNames.map((name, idx) => {
      ...
      for (const p of pts) {
        ...
        if (firstCriticalSec === null && prob >= CRITICAL_PROBABILITY_THRESHOLD) firstCriticalSec = p.time;
      }
  ...
  return (
    <div style={{ marginTop: "20px" }}>
      <div style={{
        ...
        <h3 style={{ margin: "0 0 16px", fontSize: "16px", fontWeight: 700 }}>ML Detection Results</h3>
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            <div style={{ display: "flex", height: "28px", borderRadius: "6px", overflow: "hidden", border: "1px solid #e5e7eb" }}>
              {trafficDist.green > 0.005 && (
                <div title={`Healthy: ${pct(trafficDist.green)}`} style={{ flex: trafficDist.green, backgroundColor: "#22c55e" }} />
              )}
              {trafficDist.yellow > 0.005 && (
                <div title={`Warning: ${pct(trafficDist.yellow)}`} style={{ flex: trafficDist.yellow, backgroundColor: "#f59e0b" }} />
              )}
              {trafficDist.red > 0.005 && (
                <div title={`Clogged: ${pct(trafficDist.red)}`} style={{ flex: trafficDist.red, backgroundColor: "#ef4444" }} />
              )}
            </div>
            <div style={{ display: "flex", gap: "20px", marginTop: "6px", fontSize: "12px" }}>
              <span style={{ color: "#16a34a" }}><strong>{pct(trafficDist.green)}</strong> Healthy</span>
              <span style={{ color: "#d97706" }}><strong>{pct(trafficDist.yellow)}</strong> Warning</span>
              <span style={{ color: "#dc2626" }}><strong>{pct(trafficDist.red)}</strong> Clogged</span>
            </div>
          </div>
        )}
        ...
        {mlStats && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px" }}>
            <MLStatCard label="Peak ML Probability" value={pct(mlStats.peak)} highlight={mlStats.peak >= CRITICAL_PROBABILITY_THRESHOLD} />
            <MLStatCard
              label="First Critical Alert"
              value={mlStats.firstCriticalSec !== null ? formatHHMMSS(mlStats.firstCriticalSec) : "None"}
              highlight={mlStats.firstCriticalSec !== null}
            />
            <MLStatCard
              label="Avg Probability (clogged)"
              value={mlStats.avgRedProb !== null ? pct(mlStats.avgRedProb) : "—"}
              highlight={(mlStats.avgRedProb ?? 0) >= CRITICAL_PROBABILITY_THRESHOLD}
            />
          </div>
        )}
      </div>
      ...
      {perModelStats.length > 0 && (
        <div style={{
          ...
          <div style={{ fontSize: "14px", fontWeight: 600, color: "#374151", marginBottom: "12px" }}>Per-Model Breakdown</div>
              ...
              <div key={name} style={{
                ...
                {dormant ? (
                  <div style={{ fontSize: "11px", color: "#92400e", lineHeight: 1.5 }}>
                    Not yet trained. Go to the <strong>Models</strong> tab to train this model.
                  </div>
                ) : (
      ...
      {mlChartData.length > 0 && (
        <div style={{ padding: "16px 20px", backgroundColor: "#fff", borderRadius: "8px", border: "1px solid #e5e7eb" }}>
          <div style={{ fontSize: "14px", fontWeight: 600, color: "#374151", marginBottom: "12px" }}>ML Probability Over Time</div>
          <ResponsiveContainer width="100%" height={260}>
            <ComposedChart data={mlChartData} margin={{ top: 8, right: 20, left: 0, bottom: 0 }}>
              ...
              <Area
                type="monotone" dataKey="ensemble" name="Ensemble"
                stroke="#8b5cf6" strokeWidth={2.5} fill="#8b5cf6" fillOpacity={0.12}
                dot={false} isAnimationActive={false}
              />
            </ComposedChart>
          </ResponsiveContainer>
