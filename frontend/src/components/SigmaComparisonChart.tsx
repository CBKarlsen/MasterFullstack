...
const SIGMA_COLORS: Record<number, string> = {
	3: "#3b82f6",
	4: "#f97316",
	5: "#ef4444",
};
		...
		>
			...
			<input
				...
				style={{
					...
					border: `1px solid ${isValid || !inputValue ? "#e5e7eb" : "#fca5a5"}`,
					fontSize: "13px",
				}}
			/>
			<button
				onClick={() => {
					const s = parseHHMMSS(inputValue);
					if (s !== null) onSave(s);
				}}
				disabled={!isValid}
				style={{
	...
	return (
		...
		>
			...
			<div>
				Detection onset:{" "}
				<strong>
					{row.onsetTime !== null ? formatHHMMSS(row.onsetTime) : "—"}
				</strong>
			</div>
			<div>
				Predicted ETA:{" "}
				<strong
					style={{ color: row.consensusEta !== null ? "#dc2626" : "#9ca3af" }}
				>
					{row.consensusEta !== null
						? formatHHMMSS(row.consensusEta)
...
export function SigmaComparisonChart({
	result,
	selectedFile,
	smoothingWindowSec,
}: SigmaComparisonChartProps) {
	...
	useEffect(() => {
		saveRecord(selectedFile, sigmaForecasts);
	}, [selectedFile, sigmaForecasts]);
	...
	const timelineData = useMemo(
		() => buildTimelineData(sigmaForecasts),
		[sigmaForecasts],
	);
	...
	const scatterRecords = Object.values(effectiveStore).filter(
		(r) =>
			r.actualCloggingTime !== null &&
			SIGMA_VALUES.some((s) => r.forecasts[s]?.consensusEta !== null),
	);
	...
	return (
		...
		>
			...
			<p style={{ margin: "0 0 16px", fontSize: "13px", color: "#6b7280" }}>
				Timeline shows detection onset and predicted clogging time per σ. The
				red line marks the actual (ground-truth) clogging time — enter it below
				to see accuracy.
			</p>
			...
			>
				<GroundTruthInput
					...
					currentValue={actualTime}
					onSave={handleSaveActualTime}
				/>
			</div>
				...
				>
					...
					>
						{timelineData.map((row) => (
							<Cell
								key={row.label}
								fill={SIGMA_COLORS[row.sigmaNum]}
								fillOpacity={0.85}
							/>
						))}
					</Bar>
					...
					{actualTime !== null && (
						<ReferenceLine
							...
							strokeDasharray="5 3"
							label={{
								value: `Actual: ${formatHHMMSS(actualTime)}`,
								position: "top",
								fontSize: 10,
								fill: "#dc2626",
							}}
			...
			{showScatter && <MultiFileScatter store={effectiveStore} />}
			{!showScatter && (
				<p
					style={{
						margin: "12px 0 0",
				...
				>
					Enter the actual clogging time above and click Save to generate the
	...
	const seriesData = SIGMA_VALUES.map((sigma) => ({
		...
		points: Object.values(store)
			.filter(
				(r) =>
					r.actualCloggingTime !== null &&
					r.forecasts[sigma]?.consensusEta !== null,
			)
			.map((r) => ({
				x: r.forecasts[sigma].consensusEta as number,
				y: r.actualCloggingTime as number,
				file: r.filename.split(/[\\/]/).pop() ?? r.filename,
			})),
	}));

	const allVals = seriesData.flatMap((s) =>
		s.points.flatMap((p) => [p.x, p.y]),
	);
