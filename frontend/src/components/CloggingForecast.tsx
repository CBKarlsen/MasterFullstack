...
function buildChartData(
  ...
  const step = Math.max(1, Math.floor(postOnset.length / ACTUAL_DOWNSAMPLE_MAX));
  ...
  return allTimes.map((t) => ({
    ...
    ...Object.fromEntries(Object.entries(curveMaps).map(([n, m]) => [n, m.get(t)])),
  }));
}
...
function ModelFitCard({ name, fit, isBest, criticalMultiplier }: ModelFitCardProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
        ...
        {isBest && (
          <span style={{
            fontSize: "10px", fontWeight: 700, marginLeft: "auto",
      ...
      <div style={{ fontSize: "12px", display: "flex", flexDirection: "column", gap: "4px" }}>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          ...
          <span style={{ fontWeight: 700, color: fit.r2 >= 0.8 ? "#16a34a" : fit.r2 >= 0.5 ? "#d97706" : "#dc2626" }}>
            {fit.r2.toFixed(3)}
          </span>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <span style={{ color: "#6b7280" }}>ETA to {criticalMultiplier}× threshold</span>
          <span style={{ fontWeight: 700, color: hasEta ? "#dc2626" : "#9ca3af" }}>
            {fit.eta_seconds !== null ? formatHHMMSS(fit.eta_seconds) : "No crossing"}
          </span>
        </div>
      </div>
    </div>
    ...
    <div style={{ padding: "16px 20px", backgroundColor: "#fff", borderRadius: "8px", border: "1px solid #e5e7eb" }}>
      ...
      {forecast.consensus_eta !== null ? (
        <div style={{
          ...
          <div>
            <div style={{ fontSize: "11px", color: "#6b7280" }}>Consensus ETA (median of models)</div>
            <div style={{ fontSize: "22px", fontWeight: 700, color: "#dc2626" }}>
              {formatHHMMSS(forecast.consensus_eta)}
            </div>
          </div>
          <div style={{ fontSize: "12px", color: "#9ca3af", lineHeight: 1.5 }}>
            {forecast.post_onset_points.toLocaleString()} post-onset data points used for fitting
          </div>
        </div>
      ) : (
        <div style={{
          ...
          <div style={{ fontSize: "13px", color: "#166534" }}>
            <strong>No critical crossing projected.</strong>{" "}
            The composite score is not growing toward the {forecast.critical_multiplier}× threshold.
            The partial blockage appears stable — no progressive restriction growth detected.
          </div>
        </div>
      )}
      ...
      <ResponsiveContainer width="100%" height={280}>
        <ComposedChart data={chartData} margin={{ top: 8, right: 24, left: 0, bottom: 0 }}>
          ...
          <XAxis dataKey="time" tickFormatter={(v) => formatHHMMSS(Number(v))} stroke="#9ca3af" fontSize={11} />
          <YAxis stroke="#9ca3af" fontSize={11} tickFormatter={(v) => Number(v).toFixed(4)} />
          <Tooltip
          ...
          <ReferenceLine
            y={forecast.critical_threshold} stroke="#dc2626" strokeDasharray="6 3"
            label={{ value: `${forecast.critical_multiplier}× threshold`, position: "insideTopRight", fontSize: 11, fill: "#dc2626" }}
          />
          <ReferenceLine
            y={forecast.detection_threshold} stroke="#f59e0b" strokeDasharray="4 3"
            label={{ value: "Detection", position: "insideBottomRight", fontSize: 11, fill: "#d97706" }}
          />
          <Line
            type="monotone" dataKey="actual" name="Actual composite"
            stroke="#8b5cf6" strokeWidth={1.5} dot={false} connectNulls isAnimationActive={false}
          />
          {Object.keys(MODEL_COLORS).map((name) => (
            <Line
              key={name} type="monotone" dataKey={name} name={MODEL_LABELS[name]}
              stroke={MODEL_COLORS[name]} strokeWidth={1.5} strokeDasharray="5 4"
