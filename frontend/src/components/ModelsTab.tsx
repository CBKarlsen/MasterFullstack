...
function IsolationForestTrainingPanel({ onTrainComplete }: { onTrainComplete: () => void }) {
  const [files, setFiles] = useState<DataFile[]>([]);
  const [selectedFile, setSelectedFile] = useState<string>("");
  const [sigma, setSigma] = useState(3);
  const [calibrationSeconds, setCalibrationSeconds] = useState(30);
  ...
  return (
    <div style={{ marginTop: "18px", borderTop: "1px solid #e5e7eb", paddingTop: "18px" }}>
      ...
      {/* ── Dormant warning — shown when still on synthetic data ── */}
      {(trainingInfo === null || !trainingInfo.is_user_trained) && !open && (
        <div style={{
          ...
          {" "}The model hasn't been trained on your sensor data yet. Until you train it on a healthy recording,
          all Isolation Forest predictions will be <strong>0 %</strong> and it won't affect the ensemble.
          Click <em>Train on your data</em> above to activate it.
        </div>
      )}
      ...
      {open && (
        <div style={{
          ...
          {/* Explanation of how it's different from RF */}
          <div style={{
            ...
            This is genuinely independent from the physics model.
          </div>
          ...
          {/* Parameters row */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            {/* Sigma — controls which points count as "healthy" */}
            <div>
              <label style={{ display: "block", fontSize: "13px", fontWeight: 600, marginBottom: "6px" }}>
                Healthy threshold (σ)
                <span style={{ fontSize: "11px", fontWeight: 400, color: "#9ca3af", marginLeft: "6px" }}>
          ...
          {training && progress && (
            <div style={{
              padding: "14px 16px", backgroundColor: "#f0f9ff", border: "1px solid #bae6fd",
              borderRadius: "10px",
            }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                ...
                <span style={{ fontSize: "13px", fontWeight: 700, color: "#0284c7", fontVariantNumeric: "tabular-nums" }}>
                  {progress.percent}%
                </span>
              </div>
          ...
          {result && (
            <div style={{
              ...
              <p style={{ margin: "10px 0 0", fontSize: "12px", color: "#6b7280" }}>
                The model has learned what "normal" looks like. It will now flag deviations independently of the physics thresholds.
              </p>
            </div>
          )}
        </div>
  ...
  return (
    <div style={{
      ...
      {model.name === "Isolation Forest" && (
        <IsolationForestTrainingPanel onTrainComplete={onTrainComplete} />
      )}
    </div>
  );
}
