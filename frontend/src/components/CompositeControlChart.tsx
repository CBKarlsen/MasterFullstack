...
export function CompositeControlChart({
  ...
  useEffect(() => {
    ...
    setBuffer((prev) => {
      ...
      // Maintain sliding window - shift out old data
  ...
  const currentThreshold = buffer.length > 0 ? buffer[buffer.length - 1].limit_threshold : 0.05;
          ...
          >
            ...
            <Tooltip
              formatter={(value: number, name: string) => {
                const formatted = value < 0.001 ? value.toExponential(3) : value.toFixed(4);
                return [formatted, name === 'composite_score' ? 'Composite Score' : 'Threshold'];
              }}
            ...
            <Legend
              formatter={(value) => {
                if (value === 'composite_score') return 'Composite Health Score';
