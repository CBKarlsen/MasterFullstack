...
import type { ModelMetadata, SimulationData } from "../types";
...
function probStyle(prob: number) {
	if (prob < 0.2)
		return {
			bar: COLOR.SUCCESS_BRIGHT,
			bg: COLOR.SUCCESS_SUBTLE,
			text: "#15803d",
			label: "Healthy",
		};
	if (prob < 0.7)
		return {
			bar: COLOR.WARNING,
			bg: COLOR.WARNING_BG,
			text: COLOR.WARNING_TEXT,
			label: "Warning",
		};
	return {
		bar: COLOR.DANGER_BRIGHT,
		bg: COLOR.DANGER_BG,
		text: COLOR.DANGER,
		label: "Critical",
	};
