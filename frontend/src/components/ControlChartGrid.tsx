...
export function ControlChartGrid({
  ...
  const limitedRawData = useMemo(() => {
    return rawData.length > maxDataPoints ? rawData.slice(-maxDataPoints) : rawData;
  }, [rawData, maxDataPoints]);

  const limitedChartData = useMemo(() => {
    return chartData.length > maxDataPoints ? chartData.slice(-maxDataPoints) : chartData;
  ...
  const latestThresholds = useMemo(() => {
    if (limitedChartData.length === 0) return { static: 0.018, composite: 0.05, sigma: 3.0 };
    const latest = limitedChartData[limitedChartData.length - 1] as any;
    return {
      static: latest.static_threshold > 0 ? latest.static_threshold : 0.018,
      composite: latest.limit_threshold > 0 ? latest.limit_threshold : 0.05,
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px' }}>
          {METHODS.map(({ key, label, useLog, defaultThreshold, color }) => {
            ...
            if (key === 'static') {
              threshold = methodThresholds.static ?? latestThresholds.static ?? defaultThreshold;
            } else if (key === 'composite') {
              threshold = methodThresholds.composite ?? latestThresholds.composite ?? defaultThreshold;
            } else {
