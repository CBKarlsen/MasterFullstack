...
function getEtaMinutes(
	...
	if (!isEtaPlausible(etaMin, entry.durationMin, filters.maxDurationFactor))
		return null;
		...
		>
			...
			>
				...
				<select
					...
					onChange={(e) =>
						onChange({ ...filters, maxDurationFactor: Number(e.target.value) })
					}
					style={{
						fontSize: "12px",
						padding: "2px 4px",
			...
			>
				...
				<select
					value={filters.minR2}
					onChange={(e) =>
						onChange({ ...filters, minR2: Number(e.target.value) })
					}
					style={{
						fontSize: "12px",
						padding: "2px 4px",
			...
			{excludedCount > 0 && (
				<span style={{ fontSize: "12px", color: "#d97706" }}>
					{excludedCount} ETA{excludedCount !== 1 ? "s" : ""} excluded by
					filters
				</span>
			)}
		</div>
				...
				return (
					...
					>
						...
						<div style={{ fontSize: "12px", color: "#6b7280" }}>
							MAE:{" "}
							<strong style={{ color: "#374151" }}>{mae.toFixed(1)} min</strong>
						</div>
						<div style={{ fontSize: "12px", color: "#6b7280" }}>
							Bias:{" "}
							<strong style={{ color: bias > 0 ? "#dc2626" : "#16a34a" }}>
								{bias > 0 ? "+" : ""}
								{bias.toFixed(1)} min
							</strong>
						</div>
...
export function SigmaErrorChart({ entries }: SigmaErrorChartProps) {
	const [filters, setFilters] = useState<Filters>({
		maxDurationFactor: DEFAULT_MAX_DURATION_FACTOR,
	...
	const { chartData, runs, errorsBySigma, excludedCount } = useMemo(() => {
		...
		const runList: RunMeta[] = Array.from(byLabel.keys())
			.filter((label) =>
				SIGMA_VALUES.some(
					(s) => getEtaMinutes(byLabel.get(label)!, s, filters) !== null,
				),
			)
			.map((label, i) => ({
				label,
	...
	if (!entries.some((e) => e.actualCloggingTime)) {
		return (
			...
			>
				No data yet — enter actual clogging times in the table to populate this
				chart.
			</div>
		);
	}
		...
		<div style={{ marginTop: "20px" }}>
			...
			>
				...
				<span style={{ fontWeight: 400, color: "#6b7280", marginLeft: "8px" }}>
					positive = predicted too late · negative = predicted too early · zero
					= perfect
				</span>
			</div>
			...
			{runs.length === 0 ? (
				...
				>
					All predictions excluded by current filters — try relaxing the R² or
					duration limits.
				</div>
			) : (
				<ResponsiveContainer width="100%" height={320}>
					...
					>
						...
						<YAxis
							...
							fontSize={11}
							tickFormatter={(v) =>
								`${Number(v) > 0 ? "+" : ""}${Number(v).toFixed(0)} min`
							}
							label={{
								value: "Error (min)",
								angle: -90,
