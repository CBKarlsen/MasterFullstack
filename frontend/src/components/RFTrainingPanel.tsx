      ...
      {open && (
        <div style={{
          ...
          <p style={{ margin: 0, fontSize: "13px", color: "#6b7280", lineHeight: 1.6 }}>
            ...
            <em>and</em> a clogging event. The system classifies each moment automatically.
          </p>

          <div>
            <label style={{ display: "block", fontSize: "13px", fontWeight: 600, marginBottom: "6px" }}>
              Training data
              <span style={{ fontSize: "11px", fontWeight: 400, color: "#9ca3af", marginLeft: "6px" }}>file or folder</span>
            </label>
            <select
              value={selectedFile}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            <div>
              <label style={{ display: "block", fontSize: "13px", fontWeight: 600, marginBottom: "6px" }}>
                Sensitivity (σ)
                <span style={{ fontSize: "11px", fontWeight: 400, color: "#9ca3af", marginLeft: "6px" }}>higher = fewer alerts</span>
              </label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                {N_ESTIMATORS_OPTIONS.map((o) => (
                  <label key={o.value} style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "12px", cursor: "pointer" }}>
                    <input type="radio" name="n_estimators" checked={nEstimators === o.value} onChange={() => setNEstimators(o.value)} />
                    {o.label}
                  </label>
                ))}
              </div>
            ...
            <div>
              <label style={{ display: "block", fontSize: "13px", fontWeight: 600, marginBottom: "6px" }}>Model complexity</label>
              <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                {MAX_DEPTH_OPTIONS.map((o) => (
                  <label key={String(o.value)} style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "12px", cursor: "pointer" }}>
                    <input type="radio" name="max_depth" checked={maxDepth === o.value} onChange={() => setMaxDepth(o.value)} />
                    {o.label}
                  </label>
                ))}
              </div>
          ...
          <label style={{ display: "flex", alignItems: "center", gap: "10px", fontSize: "13px", cursor: "pointer" }}>
            <input type="checkbox" checked={includeWarnings} onChange={(e) => setIncludeWarnings(e.target.checked)} style={{ width: "16px", height: "16px" }} />
            <span>
              <strong>Include warning-phase points as clogged</strong>
              <span style={{ color: "#6b7280", marginLeft: "6px" }}>— gives more training data but adds some noise</span>
            </span>
          </label>
          ...
          {training && progress && (
            <div style={{ padding: "14px 16px", backgroundColor: "#f5f3ff", border: "1px solid #c4b5fd", borderRadius: "10px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                <span style={{ fontSize: "13px", fontWeight: 600, color: "#4c1d95" }}>{PHASE_LABELS[progress.phase] ?? progress.phase}</span>
                <span style={{ fontSize: "13px", fontWeight: 700, color: "#6d28d9", fontVariantNumeric: "tabular-nums" }}>{progress.percent}%</span>
              </div>
              <div style={{ height: "8px", backgroundColor: "#ddd6fe", borderRadius: "4px", overflow: "hidden" }}>
                <div style={{ height: "100%", borderRadius: "4px", backgroundColor: "#7c3aed", width: `${progress.percent}%`, transition: "width 0.5s ease" }} />
              </div>
              <div style={{ fontSize: "11px", color: "#7c3aed", marginTop: "6px" }}>{progress.message}</div>
            </div>
          )}
          ...
          {result && (
            <div style={{ padding: "16px", backgroundColor: "#f0fdf4", border: "1px solid #86efac", borderRadius: "10px" }}>
              ...
              <div>
                <div style={{ fontSize: "12px", fontWeight: 600, color: "#374151", marginBottom: "6px" }}>What the model learned to look at:</div>
                  ...
                  {Object.entries(result.feature_importances)
                    ...
                    .map(([feature, importance]) => (
                      <div key={feature} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        ...
                        <div style={{ flex: 1, height: "8px", backgroundColor: "#dcfce7", borderRadius: "4px", overflow: "hidden" }}>
                          <div style={{ width: `${importance * 100}%`, height: "100%", backgroundColor: "#16a34a", borderRadius: "4px" }} />
                        </div>
                        <span style={{ fontSize: "12px", color: "#374151", width: "36px", textAlign: "right" }}>{pct(importance)}</span>
                      </div>
                    ))}
                </div>
              </div>
