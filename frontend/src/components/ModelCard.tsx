...
export function ModelCard({ model, prediction, onToggle }: ModelCardProps) {
  const handleToggle = async () => {
    try {
      await axios.put(`/api/models/${model.name}/enable?enabled=${!model.enabled}`);
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
        <div>
          <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 600 }}>{model.name}</h3>
          <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#6b7280' }}>
            {model.model_type} | {model.input_type}
            {model.input_type === 'sequence' && ` (${model.sequence_length} steps)`}
          </p>
        </div>
      ...
      {prediction && model.enabled && (
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '13px', color: '#6b7280' }}>Probability:</span>
            <span
              style={{
                fontSize: '18px',
                fontWeight: 'bold',
